<template>
  <div class="Members">
    ...
    >
      <el-form-item label="会员类型名称：" prop="name">
        <el-tooltip
          class="item"
          effect="dark"
          content="只能输入汉字、字母、数字、且不能以数字开头、长度小于12！"
          placement="top"
        >
          <el-input
    ...
    >
      ...
      <el-table-column prop="xuhao" label="序号" width="120" align="center">
      </el-table-column>
      <el-table-column
        prop="name"
        label="会员类型名称"
      ...
      <el-table-column prop="discount" label="折扣比例(%)" align="center">
      </el-table-column>
...
export default {
  ...
  data() {
    ...
    // 表单验证--只能为数字并且为大于0小于100的整数
    const isNumAndsmall = (rule, value, callback) => {
      const number = /^[0-9]*$/;
      if (!number.test(value)) {
        callback(new Error("折扣比例只能为大于0、小于100的整数"));
    ...
    return {
      ...
      Insertrules: {
        ...
        discount: [
          ...
          { validator: isNumAndsmall, trigger: "blur" },
        ],
      },
      // 修改功能所需要的属性
      // 是否弹出修改框
  ...
  methods: {
              ...
              if (res.status == 200) {
                ...
                for (let list = 1; list <= res.data.request.length; list++) {
                  let map = {
                    ...
                    amount: res.data.request[list - 1].amount,
                    name: res.data.request[list - 1].name,
                    discount: res.data.request[list - 1].discount,
    ...
    handleClick(row) {
      ...
      this.Updateform.discount = row.discount;
  ...
  watch: {
    ...
    InsertdialogFormVisible() {
      ...
      this.Insertform.amount = "";
      this.Insertform.discount = "";
    },
    // 检测选的页是否改变
    currentPage() {
      this.onSubmit("formInline");
