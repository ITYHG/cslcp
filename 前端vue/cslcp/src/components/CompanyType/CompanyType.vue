<template>
  <div id="CompanyType">
    ...
    >
      <el-table-column type="selection" width="220" align="center">
      </el-table-column>
      <el-table-column label="序号" width="200" prop="xuhao" align="center">
        <template slot-scope="scope">{{ scope.row.xuhao }}</template>
      </el-table-column>
      <el-table-column prop="name" label="公司类型" width="800" align="center">
      </el-table-column>
      <el-table-column
        prop="Control"
        label="操作"
    ...
    onSubmit(formInline) {
      this.$refs[formInline].validate((valid) => {
        if (valid) {
          ...
          this.$axios
            .get("/api/companytype/list", { params: parmMap })
            .then((res) => {
              if (res.status == 200) {
